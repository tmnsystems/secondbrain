...
import type { Project, Analysis, Task, PlannerOptions } from './types';
...
export async function generateTasks(
  ...
  try {
    ...
    const taskResult = await modelRequest({
 ...
 */
function parseTaskResponse(response: string): Task[] {
  try {
    ...
    const tasks: Task[] = parsedTasks.map((task: any) => ({
      id: task.id || uuidv4(),
      name: task.name,
