...
**[`uv`](https://docs.astral.sh/uv/getting-started/installation/) (optional):**
  Simplify Python environment and dependency management. `uv` can automatically create a virtual environment in the root directory and install all required packages for you—no need to manually manage Python environments. Alternatively, run `./scripts/bootstrap-env.sh`.
...
# Bootstrap Python 3.12+ venv & install dev dependencies
./scripts/bootstrap-env.sh

# Start the Deer-Flow API server (using Python 3.12+ venv)
...
```bash
# Run the project in console/UI mode (using Python API server)
python3 server.py --reload
```
