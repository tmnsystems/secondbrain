/**
 * Fixed setup script for Notion integration
 * Takes API key and page ID from environment variables
 * Fixes relation property issue
 */
...
async function setupNotion() {
  ...
  try {
    ...
    // Create tasks database with fixed relation property
      ...
      properties: {
        ...
        Project: {
          relation: {
            ...
            single_property: {} // Add this to fix validation error
          }
        },
        'Due Date': {
          date: {}
    ...
    // Create a separate database for dependencies to avoid self-relation issues
    console.log('\n📊 Creating Dependencies database...');
    const dependenciesDb = await notion.databases.create({
      parent: {
        type: 'page_id',
        page_id: rootPageId
      },
      title: [
        {
          type: 'text',
          text: {
            content: 'SecondBrain Task Dependencies'
          }
        }
      ],
      properties: {
    ...
    console.log(`✅ Dependencies database created! ID: ${dependenciesDb.id}`);
    ...
    const envVars = {
      ...
      NOTION_DEPENDENCY_DB_ID: dependenciesDb.id,
    ...
    console.log(`📝 Dependencies Database: ${dependenciesDb.id}`);
    ...
    console.log(`🔗 Dependencies Database Link: https://notion.so/${dependenciesDb.id.replace(/-/g, '')}`);
