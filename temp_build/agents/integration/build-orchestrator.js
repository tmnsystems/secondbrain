...
exports.BuildOrchestratorIntegration = void 0;
/**
 ...
 */
class BuildOrchestratorIntegration {
    /**
     ...
     */
    constructor(orchestrator, builder) {
     ...
     */
    async generateWorkflowComponents(spec) {
        try {
            // Extract specification details
            const { name, components, targetPath } = spec;
            // Generate components for the workflow
            const generatedComponents = await Promise.all(components.map(async (component) => {
     ...
     */
    async createIntegrationLayer(agents) {
        try {
            ...
            const integrationFiles = await Promise.all(Object.entries(agentsByType).map(async ([type, agentsOfType]) => {
            ...
            const indexSpec = {
                ...
                exports: Object.keys(agentsByType).map(type => ({ name: `${type}OrchestratorIntegration`, type: 'class' }))
            };
     ...
     */
    async implementCustomStepTypes(stepTypes) {
        try {
            // Generate implementation files for each custom step type
            const implementationFiles = await Promise.all(stepTypes.map(async (stepType) => {
     ...
     */
    async extendWorkflowCapabilities(capabilities) {
        try {
            // Generate implementation files for each capability
            const implementationFiles = await Promise.all(capabilities.map(async (capability) => {
     ...
     */
    getMethodsForAgentType(type, agents) {
...
exports.BuildOrchestratorIntegration = BuildOrchestratorIntegration;
