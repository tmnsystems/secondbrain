...
describe('Command Executor', () => {
    test('should successfully execute a valid command', async () => {
        const result = await (0, commandExecutor_1.executeCommand)('echo "Hello, Executor!"');
        expect(result.success).toBe(true);
        expect(result.output.trim()).toBe('Hello, Executor!');
        expect(result.exitCode).toBe(0);
    });
    test('should reject dangerous commands', async () => {
        const result = await (0, commandExecutor_1.executeCommand)('rm -rf /');
    ...
    test('should sanitize commands with injection attempts', async () => {
        const result = await (0, commandExecutor_1.executeCommand)('echo "test" && echo "injection"');
        expect(result.success).toBe(true);
    ...
    test('should enforce safe commands only mode', async () => {
        const result = await (0, commandExecutor_1.executeCommand)('custom-command', { safeCommandsOnly: true });
        expect(result.success).toBe(false);
        expect(result.error).toContain('Command rejected for security reasons');
    });
    test('should allow custom commands when safe mode is disabled', async () => {
        ...
        const result = await (0, commandExecutor_1.executeCommand)('custom-command', { safeCommandsOnly: false });
