...
exports.errorHandling = {
     ...
     */
    async handleStepError(error, execution, stepExecution, step, config) {
        try {
     ...
     */
    shouldRetry(stepExecution, retryPolicy) {
     ...
     */
    findMatchingErrorHandler(error, workflowName) {
     ...
     */
    prepareStepForRetry(stepExecution, retryPolicy) {
        // Update retry count
     ...
     * @param config Orchestrator agent configuration
     */
    recordErrorMetrics(error, execution, step, config) {
     ...
     */
    createCompensationPlan(execution, failedStepId, config) {
