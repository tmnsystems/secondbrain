...
exports.agentCoordination = void 0;
...
exports.agentCoordination = {
     ...
     * @returns Agent ID
     */
    async registerAgent(agent, capabilities, config) {
        try {
            // Generate ID if not provided
            const agentId = agent.id || (0, uuid_1.v4)();
            agent.id = agentId;
            // Store the agent
     ...
     * @param config Orchestrator agent configuration
     */
    async unregisterAgent(agentId, config) {
        try {
     ...
     */
    async getAgentCapabilities(agentId, config) {
        try {
            const info = this.agentInfo.get(agentId);
            if (!info) {
                throw new Error(`Agent not found: ${agentId}`);
     ...
     * @returns Array of agent info objects
     */
    async findAgentsByCapability(capability, config) {
        try {
    ...
    selectAgent(agents, task) {
        ...
        const availableAgents = agents.filter(a => a.status === 'online' || a.status === 'busy');
    ...
    async executeTask(task, agent) {
        try {
            ...
            try {
                ...
                // Update agent stats
                this.updateAgentStats(agent.id, true, assignment.startTime ? new Date(assignment.endTime).getTime() - new Date(assignment.startTime).getTime() : 0);
                ...
                // Update agent stats
                this.updateAgentStats(agent.id, false, assignment.startTime ? new Date(assignment.endTime).getTime() - new Date(assignment.startTime).getTime() : 0);
     ...
     */
    updateAgentStats(agentId, success, duration) {
        const info = this.agentInfo.get(agentId);
        if (!info) {
            return;
        }
     ...
     * @returns Task status
     */
    async getTaskStatus(taskId, config) {
        try {
     ...
     * @param config Orchestrator agent configuration
     */
    async completeTask(taskId, result, config) {
        try {
            const assignment = this.tasks.get(taskId);
            if (!assignment) {
                throw new Error(`Task not found: ${taskId}`);
            ...
            assignment.result = result;
            // Update agent stats
            this.updateAgentStats(assignment.agentId, true, assignment.startTime ? new Date(assignment.endTime).getTime() - new Date(assignment.startTime).getTime() : 0);
     ...
     * @param config Orchestrator agent configuration
     */
    async failTask(taskId, error, config) {
        try {
            const assignment = this.tasks.get(taskId);
            if (!assignment) {
                throw new Error(`Task not found: ${taskId}`);
            ...
            assignment.error = error;
            // Update agent stats
            this.updateAgentStats(assignment.agentId, false, assignment.startTime ? new Date(assignment.endTime).getTime() - new Date(assignment.startTime).getTime() : 0);
     ...
     */
    async checkAgentHealth(agentId, config) {
        try {
     ...
     */
    async bulkAssignTasks(tasks, config) {
        try {
            // Group tasks by capability for more efficient assignment
            const tasksByCapability = {};
            tasks.forEach(task => {
