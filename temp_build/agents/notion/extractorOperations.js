...
exports.extractorOperations = exports.extractContent = void 0;
const utils_1 = require("./utils");
/**
 * Extract content from a page in a specified format
 */
async function extractContent(client, pageId, format = 'markdown') {
    try {
        // Get the page
        ...
        return (0, utils_1.handleApiError)(error, `Failed to extract content from page: ${pageId}`);
    }
}
 ...
 */
async function getAllBlocksRecursively(client, blockId) {
    const blocks = [];
...
function getTextContent(block) {
    ...
    return block.rich_text.map((text) => text.plain_text).join('');
}
/**
 * Export all extractor operations
 */
exports.extractorOperations = {
    extractContent
};
