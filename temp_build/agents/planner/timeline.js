 ...
 */
async function createTimeline(tasks, project) {
    try {
        ...
        const timelineResult = await (0, claude_1.modelRequest)({
 ...
 */
function createTimelinePrompt(tasks, project) {
    ...
    const taskList = tasks.map(task => `- ${task.id}: ${task.name} (Priority: ${task.priority}, Effort: ${task.effort})
...
function parseTimelineResponse(response) {
    try {
        ...
        const timeline = {
            ...
            milestones: (parsedTimeline.milestones || []).map((milestone) => ({
                name: milestone.name,
                date: milestone.date,
