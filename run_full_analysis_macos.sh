...
# macOS compatible version that doesn't rely on timeout command
...
# Run with time limit function (macOS alternative to timeout)
run_with_time_limit() {
    local cmd="$1"
    local time_limit="$2"
    ...
    # If we get here, we need to kill the process
    echo "Process taking too long, stopping it..."
    kill -TERM $pid 2>/dev/null || kill -KILL $pid 2>/dev/null
    return 143  # Similar to timeout exit code
}
...
while true; do
    ...
    # Run with a time limit for macOS (alternative to timeout)
    run_with_time_limit "node process_in_batches.js process" $MAX_RUNTIME_SECONDS
